...
import { FiMessageSquare, FiFolder, FiShoppingCart, FiUser } from "react-icons/fi";
...
const DashboardLayout: FC<CommanLayoutProps> = ({ children }) => {
  const menus = [
    ...
    { name: "Profile", link: "/dashboard/profile", icon: FiUser, margin: true },
  ];
        ...
        <div className="mt-4 flex flex-col gap-4 relative">
          {menus?.map((menu, i) => (
            <Link
              href={menu?.link}
              key={i}
              className={`${pathName===menu?.link? 'bg-gray-400':''} ${
                menu?.margin && "mt-5"
